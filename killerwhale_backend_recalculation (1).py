...
LEGEND_MASK_X_PERCENT = 70.0  # Manual mask starts here and continues to the right edge


def _manual_legend_mask_picker(pix, default_x, y0, y1, title="Mask Legend"):
    """
    Web-safe legend mask selector.

    KillerWhale Way:
        Uses the extractor's original automatically detected X position.

    Manual Way:
        Uses LEGEND_MASK_X_PERCENT from the Streamlit slider.
        The extractor still uses its detected legend-row Y range, matching
        the old desktop Manual Way behavior where only the legend row is masked.
    """
    if not MANUAL_LEGEND_MASK:
        return int(default_x)

    pct = globals().get("LEGEND_MASK_X_PERCENT", None)
    if pct is None:
        return int(default_x)
    ...
    return int(round(pix.width * pct / 100.0))
