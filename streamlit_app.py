...
legend_mode = st.radio(
    "Masking method",
    options=["KillerWhale Way", "Manual Way"],
    horizontal=True,
    label_visibility="collapsed",
)

manual_mask_percent = 70.0
if legend_mode == "Manual Way":
    st.caption(
        "Move the slider to choose where the legend mask starts. "
        "Only the detected legend row is removed from this point to the right edge."
    )
    manual_mask_percent = st.slider(
        "Manual mask start position",
        min_value=0.0,
        max_value=100.0,
        value=70.0,
        step=0.5,
...
if st.button("EXTRACT", type="primary", use_container_width=True):
    if not (re_files or ce_pdf_files or ce_excel_files or oats_files):
        st.warning("Select RE PDF(s), CE PDF + Excel pair(s), or Recalculation PDF(s) first.")
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="killerwhale_web_") as tmp:
                ...
                kw.LEGEND_MASK_X_PERCENT = manual_mask_percent
