...
if legend_mode == "Manual Way":
    ...
    if previews:
        ...
        for i, p in enumerate(previews):
            ...
            # Use a real temporary PNG path for the click component.
            preview_hash = hashlib.sha1(p["png"]).hexdigest()[:12]
            preview_path = Path(tempfile.gettempdir()) / f"killerwhale_mask_{i}_{preview_hash}.png"
            if not preview_path.exists():
                preview_path.write_bytes(p["png"])

            # Always show the graph normally as well.
            st.image(str(preview_path), width=p["display_width"])
