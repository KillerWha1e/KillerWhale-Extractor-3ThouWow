...
MANUAL_LEGEND_MASK = False  # Web backend: keep GUI masking disabled


def _manual_legend_mask_picker(pix, default_x, y0, y1, title="Mask Legend"):
    """
    Show a draggable mask preview and return the selected X pixel.

    Returns:
      int  -> mask from this X to the right edge
      None -> Skip Mask

    The Y range is supplied by the extractor from the detected legend row,
    so moving the bar does NOT white-out the whole graph vertically.
    """
    ...
    # Fit a large graph comfortably on a normal desktop while preserving
    # the original pixel coordinate returned to the masking code.
    max_w, max_h = 1200, 700
    preview_scale = min(max_w / pix.width, max_h / pix.height, 1.0)
    disp_w = max(1, int(round(pix.width * preview_scale)))
    disp_h = max(1, int(round(pix.height * preview_scale)))

    if preview_scale < 1.0:
        preview = raw.resize((disp_w, disp_h), Image.Resampling.LANCZOS)
    else:
        preview = raw
    ...
    tk.Label(
        win,
        text=(
            "Drag the RED bar left/right. The white preview is the part of the "
            "legend row that will be removed. Press ENTER to apply."
        ),
    ...
    tk.Label(
        win,
        text="Only this legend row will be masked — the graph above/below it stays unchanged.",
        font=("Segoe UI", 9)
    ...
    sy0 = max(0, min(disp_h, int(round(y0 * preview_scale))))
    sy1 = max(0, min(disp_h, int(round(y1 * preview_scale))))
    if sy1 <= sy0:
        sy1 = min(disp_h, sy0 + max(8, int(18 * preview_scale)))

    selected = {
        "x": max(0, min(pix.width, int(default_x))),
        "result": None,
    }

    def original_to_display_x(x):
        return max(0, min(disp_w, int(round(x * preview_scale))))

    def display_to_original_x(x):
        if preview_scale <= 0:
            return int(default_x)
        return max(0, min(pix.width, int(round(x / preview_scale))))
    ...
    def redraw():
        nonlocal overlay_id, line_id
        if overlay_id is not None:
            canvas.delete(overlay_id)
        if line_id is not None:
            canvas.delete(line_id)

        sx = original_to_display_x(selected["x"])
        # White rectangle gives an exact visual preview of the final mask.
        overlay_id = canvas.create_rectangle(
            sx, sy0, disp_w, sy1,
            fill="white", outline="white"
        )
        line_id = canvas.create_line(
            sx, max(0, sy0 - 16), sx, min(disp_h, sy1 + 16),
            fill="red", width=3
        )
        canvas.tag_raise(line_id)
        pct = 100.0 * selected["x"] / max(1, pix.width)
        value_var.set(f"Mask starts at X = {selected['x']} px  ({pct:.1f}% of graph width)")

    def move_bar(event):
        selected["x"] = display_to_original_x(event.x)
        redraw()

    def apply_mask():
        selected["result"] = int(selected["x"])
        win.destroy()
    ...
    def reset_bar():
        selected["x"] = max(0, min(pix.width, int(default_x)))
        redraw()

    canvas.bind("<Button-1>", move_bar)
    canvas.bind("<B1-Motion>", move_bar)

    tk.Label(win, textvariable=value_var, font=("Segoe UI", 9)).pack(pady=(5, 2))

    buttons = tk.Frame(win)
    buttons.pack(pady=(4, 12))
    tk.Button(buttons, text="Reset", command=reset_bar, width=12).pack(side="left", padx=4)
    tk.Button(buttons, text="Skip Mask", command=skip_mask, width=12).pack(side="left", padx=4)
    tk.Button(
        buttons, text="Apply Mask", command=apply_mask,
        width=14, font=("Segoe UI", 9, "bold")
    ).pack(side="left", padx=4)

    # Keyboard shortcuts for faster manual masking.
    # ENTER = same as clicking Apply Mask.
    # ESC = same as Skip Mask.
    win.bind("<Return>", lambda event: apply_mask())
    win.bind("<KP_Enter>", lambda event: apply_mask())
    win.bind("<Escape>", lambda event: skip_mask())

    win.protocol("WM_DELETE_WINDOW", skip_mask)
    win.focus_force()
    redraw()
    win.update_idletasks()

    # Center approximately on screen.
    try:
        x = max(0, (win.winfo_screenwidth() - win.winfo_reqwidth()) // 2)
        y = max(0, (win.winfo_screenheight() - win.winfo_reqheight()) // 2)
        win.geometry(f"+{x}+{y}")
    except Exception:
        pass
...
def write_oats_sheet(wb, all_results, class_type):
    ...
    ws = wb.create_sheet("OATS Conversion")
    ws.sheet_view.zoomScale = 75
    ws.sheet_view.zoomScaleNormal = 75
    ws.merge_cells("A1:K1")
    ws["A1"] = f"OATS Converted Data — Class {class_type}"
...
def save_combined_excel(results, out_xlsx, ce_results_list=None, oats_results=None, oats_class="A"):
    ...
    # Add OATS as a third tab when OATS PDFs were selected.
    if oats_results:
...
def extract_multiple_pdfs(pdf_paths, ce_pdf_paths=None, ce_excel_paths=None, oats_pdf_paths=None, oats_class="A", progress_callback=None):
    ...
    if not pdf_paths and not ce_pairs and not oats_pdf_paths:
        raise ValueError("Select RE PDF(s), CE PDF + Excel pair(s), or OATS PDF(s).")
    ...
    with tempfile.TemporaryDirectory(prefix="emc_graphs_") as temp_dir:
        ...
        for oats_index, oats_pdf_path in enumerate(oats_pdf_paths, 1):
            report_progress(
                f"Extracting OATS PDF {oats_index}/{len(oats_pdf_paths)}: {oats_pdf_path.name}"
            )
            ...
            completed_steps += 1
            report_progress(f"Finished OATS PDF {oats_index}/{len(oats_pdf_paths)}")
